...
-- Earlier builds parented a BlurEffect to Lighting and an acrylic Part to the
-- Camera. Both live outside the ScreenGui, so destroying the interface never
-- removed them: if one of those builds ran this session, its effect is still
-- applied and nothing else will clear it short of rejoining. Sweep them here.
local function clearLegacyEffects()
    pcall(function()
        for _, object in game:GetService("Lighting"):GetChildren() do
            if object:IsA("BlurEffect") and object.Name == "TRustMenuBlur" then
                object:Destroy()
            end
        end
    end)
    ...
    pcall(function()
        ...
        for _, object in activeCamera:GetChildren() do
            if object.Name == "TRustMenuAcrylic" then
                object:Destroy()
            end
        end
    end)
