...
local Fonts = {
    ...
    Semibold = robotoFont(Enum.FontWeight.SemiBold),
...
local Motion = {
    ...
    Layout = 0.24, -- position, size, open-close
}

local Library = {
    Version = "1.2.1-dev",
    Build = "fix-control-visibility-v1.2.1",
    FontName = "Roboto",
...
function Library:CreateWindow(options)
    ...
    bindTheme(sidebarDivider, "BackgroundColor3", function(theme) return theme.Divider end)
    ...
    bindTheme(topbarDivider, "BackgroundColor3", function(theme) return theme.Divider end)
    ...
    -- Floating launcher. It sits outside MainWindow so it survives the hide
    -- animation and stays reachable while the menu is closed.
    local launcherSize = options.ToggleButtonSize or 52
    local launcherIconSize = floor(launcherSize * 0.56)
    local launcherPressSize = floor(launcherIconSize * 0.84)
    local launcherX = options.ToggleButtonPosition and options.ToggleButtonPosition.X or 24
    local launcherY = options.ToggleButtonPosition and options.ToggleButtonPosition.Y
        or max(24, viewport.Y - launcherSize - 32)

    local launcher = create("Frame", {
        Parent = screenGui,
        Name = "MenuLauncher",
        Position = fromOffset(launcherX, launcherY),
        Size = fromOffset(launcherSize, launcherSize),
        BackgroundTransparency = 1,
        Visible = options.ToggleButton ~= false,
        ZIndex = 200,
    })

    local launcherGlow = create("Frame", {
        Parent = launcher,
        Name = "Glow",
        AnchorPoint = Vector2.new(0.5, 0.5),
    ...
    corner(launcherGlow, floor((launcherSize + 14) / 2))
    bindTheme(launcherGlow, "BackgroundColor3", function(theme) return theme.Accent end)
    ...
    corner(launcherButton, floor(launcherSize / 2))
    local launcherStroke = stroke(launcherButton, Theme.Accent, 0.25, 2)
    bindTheme(launcherButton, "BackgroundColor3", function(theme) return theme.Card end)
    bindTheme(launcherStroke, "Color", function(theme) return theme.Accent end)

    local launcherIcon = create("ImageLabel", {
        Parent = launcherButton,
        Name = "Icon",
        AnchorPoint = Vector2.new(0.5, 0.5),
        Position = UDim2.fromScale(0.5, 0.5),
        Size = fromOffset(launcherIconSize, launcherIconSize),
        BackgroundTransparency = 1,
        Image = logoImage,
        ImageColor3 = Theme.Accent,
        Visible = logoImage ~= "",
    ...
    bindTheme(launcherIcon, "ImageColor3", function(theme) return theme.Accent end)

    local launcherGlyph = create("TextLabel", {
        Parent = launcherButton,
        Name = "Glyph",
        Size = UDim2.fromScale(1, 1),
        BackgroundTransparency = 1,
    ...
    local window = {
        ...
        LauncherStroke = launcherStroke,
        LauncherIcon = launcherIcon,
        LauncherGlyph = launcherGlyph,
        GlowBindings = {},
    ...
    function window:SetOpacity(value, animate)
        ...
        -- While hidden the group transparency belongs to the close animation.
        -- Store the value and let SetVisible apply it on the way back in.
    ...
    local VISIBILITY_SHRINK = 0.94
    local VISIBILITY_LIFT = 12
    local VISIBILITY_IN = 0.30
    local VISIBILITY_OUT = 0.20
    ...
    -- The scale pivots on the top-left corner, so the closed position is
    -- nudged inwards by half the shrink to make the motion read as centred.
    local function closedPosition(resting)
        return UDim2.new(
            resting.X.Scale,
            resting.X.Offset + floor(main.AbsoluteSize.X * (1 - VISIBILITY_SHRINK) / 2),
            resting.Y.Scale,
            resting.Y.Offset + floor(main.AbsoluteSize.Y * (1 - VISIBILITY_SHRINK) / 2) + VISIBILITY_LIFT
        )
    end
    ...
    function window:FitToViewport()
        ...
        if self.Visible == false then
            self.Main.Position = closedPosition(self.RestingPosition)
        else
        ...
        if launcherFrame and launcherFrame.Parent then
            launcherFrame.Position = fromOffset(
                clamp(launcherFrame.Position.X.Offset, 4, max(4, liveViewport.X - launcherSize - 4)),
                clamp(launcherFrame.Position.Y.Offset, 4, max(4, liveViewport.Y - launcherSize - 4))
            )
        end
    ...
    function window:RefreshLauncher(animate)
        if not launcher.Parent then return end

        local open = self.Visible == true
        local glowSize = open and UDim2.new(1, 8, 1, 8) or UDim2.new(1, 14, 1, 14)
        local glowTransparency = open and 0.94 or 0.84
        local strokeTransparency = open and 0.6 or 0.22
        local iconSize = open and floor(launcherIconSize * 0.9) or launcherIconSize

        if animate then
            tween(launcherGlow, {Size = glowSize, BackgroundTransparency = glowTransparency}, Motion.State, Enum.EasingStyle.Quart)
            tween(launcherStroke, {Transparency = strokeTransparency}, Motion.State, Enum.EasingStyle.Quart)
            tween(launcherIcon, {Size = fromOffset(iconSize, iconSize)}, Motion.State, Enum.EasingStyle.Quart)
        else
            setProperties(launcherGlow, {Size = glowSize, BackgroundTransparency = glowTransparency})
    ...
    function window:SetVisible(state, animate)
        ...
        local resting = self.RestingPosition or self.Main.Position
        local closed = closedPosition(resting)
        self:RefreshLauncher(animate)

        if not animate then
            setProperties(self.WindowScale, {Scale = state and 1 or VISIBILITY_SHRINK})
            setProperties(self.Main, {
                GroupTransparency = state and openTransparency() or 1,
                Position = state and resting or closed,
            })
        ...
        if state then
            -- Snap to the closed pose first so the opening tween always starts
            -- from the same place, even if a close is still mid-flight.
            setProperties(self.WindowScale, {Scale = VISIBILITY_SHRINK})
            setProperties(self.Main, {GroupTransparency = 1, Position = closed})
            setProperties(self.PopupLayer, {GroupTransparency = 1})
            self.Main.Visible = true
            self.PopupLayer.Visible = true

            tween(self.WindowScale, {Scale = 1}, VISIBILITY_IN, Enum.EasingStyle.Quint)
            tween(self.Main, {GroupTransparency = openTransparency(), Position = resting}, VISIBILITY_IN, Enum.EasingStyle.Quint)
            tween(self.PopupLayer, {GroupTransparency = openTransparency()}, VISIBILITY_IN, Enum.EasingStyle.Quint)
        else
            tween(self.WindowScale, {Scale = VISIBILITY_SHRINK}, VISIBILITY_OUT, Enum.EasingStyle.Quart)
            tween(self.Main, {GroupTransparency = 1, Position = closed}, VISIBILITY_OUT, Enum.EasingStyle.Quart)
            tween(self.PopupLayer, {GroupTransparency = 1}, VISIBILITY_OUT, Enum.EasingStyle.Quart)
    ...
    function window:SetToggleKey(key)
        if typeof(key) == "EnumItem"
            and (key.EnumType == Enum.KeyCode or key.EnumType == Enum.UserInputType) then
            self.ToggleKey = key
        end
    ...
    function window:SetLauncherVisible(state)
        state = state ~= false
        launcher.Visible = state
        return state
    end

    function window:IsLauncherVisible()
        return launcher.Visible
    end

    -- Tap versus drag is decided by distance, so the button can be repositioned
    -- without firing a toggle when the finger lifts.
    local launcherDragging = false
    local launcherMoved = false
    local launcherStart
    local launcherOrigin

    local function launcherPress(pressed)
        local size = pressed and launcherPressSize or launcherIconSize
        tween(launcherIcon, {Size = fromOffset(size, size)}, Motion.Press, Enum.EasingStyle.Sine)
    end
    ...
    launcherButton.InputBegan:Connect(function(input)
        ...
        launcherOrigin = launcher.Position
        launcherPress(true)
    end)

    launcherButton.MouseEnter:Connect(function()
        if launcherDragging then return end
        tween(launcherStroke, {Transparency = 0.08}, Motion.State, Enum.EasingStyle.Quart)
    end)

    launcherButton.MouseLeave:Connect(function()
        if launcherDragging then return end
        window:RefreshLauncher(true)
    end)
    ...
    connect(UserInputService.InputChanged, function(input)
        ...
        local liveCamera = Workspace.CurrentCamera or Camera
        local liveViewport = liveCamera and liveCamera.ViewportSize or viewport
        launcher.Position = fromOffset(
            clamp(launcherOrigin.X.Offset + delta.X, 4, max(4, liveViewport.X - launcherSize - 4)),
            clamp(launcherOrigin.Y.Offset + delta.Y, 4, max(4, liveViewport.Y - launcherSize - 4))
        )
    end)
            ...
            function tab:Section(sectionOptions)
                ...
                function sectionObject:Refresh()
                    ...
                    task.defer(function()
                        ...
                        self.Frame.Size = UDim2.new(1, -4, 0, 82 + contentHeight)
...
function SectionMethods:AddToggle(options)
    options = options or {}
    local text = options.Text or options.Name or options.Title or options.Label or "Toggle"
    ...
    function control:ApplyTheme(animate)
        ...
        if animate then
            tween(self.Switch, {BackgroundColor3 = switchColor}, Motion.State, Enum.EasingStyle.Quart)
            tween(self.SwitchStroke, {Color = strokeColor}, Motion.State, Enum.EasingStyle.Quart)
            tween(self.Label, {TextColor3 = labelColor}, Motion.State, Enum.EasingStyle.Quart)
            tween(self.Glow, {BackgroundColor3 = Theme.Accent}, Motion.State, Enum.EasingStyle.Quart)
            for _, shadow in self.KnobShadows do
                tween(shadow, {BackgroundColor3 = Theme.Shadow}, Motion.State, Enum.EasingStyle.Quart)
            end
            tween(self.KnobStroke, {Color = knobStrokeColor}, Motion.State, Enum.EasingStyle.Quart)
        else
    ...
    function control:ApplyVisual(animate)
        ...
        if animate then
            tween(self.Glow, {BackgroundTransparency = glowTransparency}, Motion.State, Enum.EasingStyle.Quart)
            tween(self.Knob, {Position = knobPosition}, Motion.Layout, Enum.EasingStyle.Quint)
            for index, shadow in self.KnobShadows do
                tween(shadow, {BackgroundTransparency = shadowTransparency[index]}, Motion.Layout, Enum.EasingStyle.Quart)
            end
        else
    ...
    row.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1
            or input.UserInputType == Enum.UserInputType.Touch then
            tween(knob, {Size = fromOffset(23, 23)}, Motion.Press, Enum.EasingStyle.Sine)
            tween(switchGlow, {Size = fromOffset(62, 38)}, Motion.Press, Enum.EasingStyle.Sine)
        end
    end)
    ...
    row.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1
            or input.UserInputType == Enum.UserInputType.Touch then
            tween(knob, {Size = fromOffset(20, 20)}, Motion.State, Enum.EasingStyle.Back)
            tween(switchGlow, {Size = fromOffset(58, 34)}, Motion.State, Enum.EasingStyle.Back)
        end
    end)
...
function SectionMethods:AddSlider(options)
    options = options or {}
    local text = options.Text or options.Name or options.Title or options.Label or "Slider"
    local callback = options.Callback or options.OnChanged or options.Function
    ...
    function control:Set(value, emit, animate, visualRatio)
        ...
        if shouldAnimate then
            tween(self.Fill, {Size = fillSize}, Motion.State, Enum.EasingStyle.Quart)
            tween(self.Glow, {Position = thumbPosition}, Motion.State, Enum.EasingStyle.Quart)
        else
    ...
    hitbox.MouseEnter:Connect(function()
        if not dragging then tween(thumbGlow, {Size = fromOffset(33, 33)}, Motion.Press, Enum.EasingStyle.Sine) end
    end)
    hitbox.MouseLeave:Connect(function()
        if not dragging then tween(thumbGlow, {Size = fromOffset(30, 30)}, Motion.Press, Enum.EasingStyle.Sine) end
    end)
    ...
    hitbox.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1
            ...
            }, Motion.Press, Enum.EasingStyle.Back)
            tween(knob, {Size = fromOffset(21, 21)}, Motion.Press, Enum.EasingStyle.Back)
    ...
    connectControl(control, UserInputService.InputEnded, function(input)
        ...
        if dragging and (mouseEnded or touchEnded) then
            ...
            tween(fill, {Size = UDim2.new(ratio, 0, 1, 0)}, Motion.Press, Enum.EasingStyle.Quart)
            tween(thumbGlow, {Position = UDim2.new(ratio, 0, 0.5, 0)}, Motion.Press, Enum.EasingStyle.Quart)
            tween(thumbGlow, {
                Size = fromOffset(30, 30),
                BackgroundTransparency = 0.84,
            }, Motion.State, Enum.EasingStyle.Back)
            tween(knob, {Size = fromOffset(18, 18)}, Motion.State, Enum.EasingStyle.Back)
...
function SectionMethods:AddDropdown(options)
    options = options or {}
    local text = options.Text or options.Name or options.Title or options.Label or "Dropdown"
    local callback = options.Callback or options.OnChanged or options.Function
    local flag = self:NextFlag(options.Flag)
    ...
    function control:Set(value, emit)
        ...
        self.Value = value
        self.Field.Text = ""
        self.ValueLabel.Text = value == nil and tostring(options.Placeholder or "Select...") or tostring(value)
        self.ValueLabel.TextColor3 = value == nil and Theme.Dim or Theme.Text
        self.Section.Window.Flags[self.Flag] = value
        Library.Flags[self.Flag] = value
...
function SectionMethods:AddTextbox(options)
    options = options or {}
    local text = options.Text or options.Name or options.Title or options.Label or "Textbox"
    local onChanged = options.OnChanged or options.Changed
    local onSubmitted = options.OnSubmitted or options.Callback or options.Function
    local flag = self:NextFlag(options.Flag)
...
function SectionMethods:AddButton(options)
    options = options or {}
    local text = options.Text or options.Name or options.Title or options.Label or "Button"
    local callback = options.Callback or options.OnClick or options.Function
    ...
    -- UIGradient also multiplies a TextButton's own glyph color. Render the
    -- caption as a child label so button names remain bright and readable.
    ...
    function control:SetEnabled(value)
        ...
        tween(button, {BackgroundTransparency = self.Enabled and 0 or 0.35}, Motion.State)
    end
    ...
    button.MouseEnter:Connect(function()
        if control.Enabled then
            tween(buttonStroke, {Color = Theme.AccentSoft}, Motion.Press)
        end
    end)
    button.MouseLeave:Connect(function()
        tween(buttonStroke, {Color = Theme.Border}, Motion.Press)
    end)
...
function SectionMethods:AddLabel(options)
    ...
    local text = tostring(options.Text or options.Value or options.Name or "Label")
    local flag = options.Flag and self:NextFlag(options.Flag) or nil
    local label = create("TextLabel", {
        Parent = self.Content,
    ...
    if flag then
        control.Setter = registerSetter(self, flag, function(value)
...
function SectionMethods:AddKeybind(options)
    options = options or {}
    local text = options.Text or options.Name or options.Title or options.Label or "Keybind"
    local callback = options.Callback or options.OnTriggered or options.Function
    ...
    bindButton.Activated:Connect(function()
        ...
        bindButton.Text = "..."
        tween(bindButton, {TextColor3 = Theme.Accent}, Motion.Press)
    end)
...
function SectionMethods:AddColorPicker(options)
    options = options or {}
    local text = options.Text or options.Name or options.Title or options.Label or "Color"
    local callback = options.Callback or options.OnChanged or options.Function
    ...
    function control:Close()
        ...
        draggingMode = nil
        activeTouch = nil
        tween(self.FieldStroke, {Color = Theme.Border}, Motion.State, Enum.EasingStyle.Quart)
    ...
    function control:Open()
        ...
        setProperties(self.PopupScale, {Scale = 0.97})
        self.Popup.Visible = true
        tween(self.PopupScale, {Scale = 1}, Motion.State, Enum.EasingStyle.Back)
        tween(self.FieldStroke, {Color = Theme.AccentSoft}, Motion.State, Enum.EasingStyle.Quart)
        self.Window.OpenPopup = self
    end
    ...
    local function beginDrag(mode, input)
        ...
        if mode == "sv" then
            tween(svSelector, {Size = fromOffset(18, 18)}, Motion.Press, Enum.EasingStyle.Back)
        else
            tween(hueSelector, {Size = fromOffset(9, 24)}, Motion.Press, Enum.EasingStyle.Back)
        end
    ...
    connectControl(control, UserInputService.InputEnded, function(input)
        ...
        if draggingMode and (mouseEnded or touchEnded) then
            ...
            draggingMode = nil
            activeTouch = nil
            tween(svSelector, {Size = fromOffset(14, 14)}, Motion.State, Enum.EasingStyle.Back)
            tween(hueSelector, {Size = fromOffset(6, 22)}, Motion.State, Enum.EasingStyle.Back)
    ...
    field.MouseEnter:Connect(function()
        tween(field, {BackgroundColor3 = Theme.ControlHover}, Motion.Press, Enum.EasingStyle.Quart)
    end)
    field.MouseLeave:Connect(function()
        tween(field, {BackgroundColor3 = Theme.Control}, Motion.Press, Enum.EasingStyle.Quart)
    end)
    field.Activated:Connect(function()
    ...
    onThemeChanged(control, function(_, animate)
        ...
        if animate then
            tween(fieldStroke, {Color = fieldColor}, Motion.State, Enum.EasingStyle.Quart)
        else
